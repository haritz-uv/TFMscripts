...
def main():
    ...
    # data-selection
    nombre_microscopio = seleccionar_opcion(calibration.keys(), "Microscope")
    mis_lentes = calibration[nombre_microscopio]
    magnificacion = seleccionar_opcion(mis_lentes.keys(), "Magnification used")
    factor = mis_lentes[magnificacion]
    print(f"\nℹ️  pixel to um factor: {factor} px/µm")
    ...
    texto_usuario = input("Image path: ")
    ...
    try:
        ...
        nombre_salida = os.path.join(dir_path, f"scaled_{filename}")
        img.save(nombre_salida)
        print(f"\nIMG saved as : {nombre_salida}")
        print("Success")
